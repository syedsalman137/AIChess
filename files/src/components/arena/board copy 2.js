...
function Chessboard() {
    ...
    function pushPieces(move: string, ifRefresh: number = 1) {
        let check_move = move + '+';
        let check_mate_move = move + '#';
        let isPromo = move.includes('=');
        let allowedMovesForPiece = null;
        if (secondBackPiece === null) {
            allowedMovesForPiece = chess.moves({ square: selectedPiece.id });
        }
        else {
            allowedMovesForPiece = chess.moves({ square: secondBackPiece.id });
        }
        let movePlayed = false;
        if (!chess.isGameOver()) { 
            if (allowedMovesForPiece.includes(move) | allowedMovesForPiece.includes(move.slice(0, 1) + move.slice(2)) | allowedMovesForPiece.includes(move.slice(0, 1) + move.slice(3)) ) {
                movePlayed = chess.move(move);
                if (movePlayed !== null) {
                    movePlayed = True;
                }
                console.log(2)
            ...
            else if (allowedMovesForPiece.includes(check_move) | allowedMovesForPiece.includes(check_move.slice(0, 1) + check_move.slice(2)) | allowedMovesForPiece.includes(check_move.slice(0, 1) + check_move.slice(3)) ) {
                movePlayed = chess.move(check_move);
                if (movePlayed !== null) {
                    movePlayed = True;
                }
                console.log(3)
            ...
            else if (allowedMovesForPiece.includes(check_mate_move) | allowedMovesForPiece.includes(check_mate_move.slice(0, 1) + check_mate_move.slice(2)) | allowedMovesForPiece.includes(check_mate_move.slice(0, 1) + check_mate_move.slice(3)) ) {
                movePlayed = chess.move(check_mate_move);
                if (movePlayed !== null) {
                    movePlayed = True;
                }
                movePlayed = true;
    ...
    function grabPiece(e: React.MouseEvent) {
        ...
        if (element.classList.contains('piece')) {
            if (/\W\w{2}\./.exec(element.style.backgroundImage)[0].charAt(1) == 'w') {
                ...
                let isKilled = tryKillPiece(element);
                if (isKilled) {
                    selectedPiece = element;
                    console.log("Done")
                ...
                if (element.classList.contains('tile')) {
                    ...
                    if (pieceName !== 'p') {
                        ...
                        if (isPrePromotionTile) {
                            ...
                            var move = [element.id];
                            move.push(selectedPiece.id.charAt(0) + 'x' + element.id);   //enpassant
                        }
                    }
                    // console.log(chess.moves())
                    if (move !== null) {
                    ...
                    console.log(secondBackPiece.id)
                    console.log(selectedPiece.id)
                    console.log(element.id)
                    let str = /\W\w{2}\./.exec(element.style.backgroundImage);
                    let pieceName = str[0].charAt(2);
                    // var move = [selectedPiece.id + "=" + pieceName.toUpperCase()];
                    var move = secondBackPiece.id.charAt(0) + 'x' + selectedPiece.id + "=" + pieceName.toUpperCase();
                    console.log(move)
                    pushPieces(move);
                }
                selectedPiece.style.border = 'none';
